...
def send_to_api(cal, start_h, start_m, end_h, end_m, entry_cons_id, entry_cust_id, var_lunch):
...
def gui():
    ...
    start_h = tk.Spinbox(time_frame, 
                         from_=0, 
                         to=23, 
                         width=3, 
                         format="%02.0f"); start_h.grid(row=0, column=1)

    start_m = tk.Spinbox(time_frame, 
                         from_=0, 
                         to=59, 
                         width=3, 
                         format="%02.0f"); start_m.grid(row=0, column=2)

    tk.Label(time_frame, text="End:").grid(row=1, column=0)
    end_h = tk.Spinbox(time_frame, 
                       from_=0, 
                       to=23, 
                       width=3, 
                       format="%02.0f"); end_h.grid(row=1, column=1)
    end_m = tk.Spinbox(time_frame, 
                       from_=0, 
                       to=59, 
                       width=3, 
                       format="%02.0f"); end_m.grid(row=1, column=2)

    # Lunchbreak
    var_lunch = tk.BooleanVar()
    tk.Checkbutton(root, 
                   text="Lunch Break Included?", 
                   variable=var_lunch).pack(pady=10)

    # Submit Button
    btn_send = tk.Button(
    root, 
    text="Log Hours to Azure", 
    command=lambda: send_to_api(cal, start_h, start_m, end_h, end_m, entry_cons_id, entry_cust_id, var_lunch),
    bg="#0078d4", 
    fg="white", 
    ...
    root.mainloop()
    # return cal, start_h, start_m, end_h, end_m, entry_cons_id, entry_cust_id, var_lunch

def gui_2():
    root = tk.Tk()
    root.title("NEURAL-LINK: TIME MANAGEMENT")
    root.geometry("400x600")
    
    # Futuristic Styling Configuration
    bg_color = "#FFFFFF"
    accent_color = "#0F50C9"
    text_color = "#0F50C9"
    secondary_bg = "#CDDAE4"
    ...
    # --- Header ---
    tk.Label(root, text="Time management system", bg=bg_color, fg=accent_color, 
             font=('Courier', 14, 'bold')).pack(pady=20)

    # --- Input Fields ---
    tk.Label(root, text="CONSULTANT ID", bg=bg_color, fg=text_color, font=('Courier', 10)).pack()
    entry_cons_id = tk.Entry(root, bg=secondary_bg, fg=accent_color, insertbackground=accent_color,
                             relief="flat", borderwidth=5)
    entry_cons_id.pack(pady=5)

    tk.Label(root, text="CUSTOMER ID", bg=bg_color, fg=text_color, font=('Courier', 10)).pack()
    entry_cust_id = tk.Entry(root, bg=secondary_bg, fg=accent_color, insertbackground=accent_color,
                             relief="flat", borderwidth=5)
    entry_cust_id.pack(pady=5)

    # --- Calendar (tkcalendar supports styling too) ---
    tk.Label(root, text="TEMPORAL DATE", bg=bg_color, fg=text_color, font=('Courier', 10)).pack(pady=5)
    cal = DateEntry(root, width=12, background=secondary_bg, foreground=accent_color, 
                    borderwidth=2, date_pattern='yyyy-mm-dd')
    cal.pack(pady=5)

    # --- Time Selectors Frame ---
    time_frame = tk.LabelFrame(root, text="WORK CYCLE", bg=bg_color, fg=accent_color, 
                               font=('Courier', 10), padx=10, pady=10, relief="flat", highlightbackground=accent_color, highlightthickness=1)
   ...
   # Time Selectors (Spinboxes)
    ...
    # Style the spinboxes inside the frame
    spin_style = {"bg": secondary_bg, "fg": accent_color, "buttonbackground": secondary_bg, "relief": "flat"}
    
    tk.Label(time_frame, text="START:", bg=bg_color, fg=text_color).grid(row=0, column=0)
    start_h = tk.Spinbox(time_frame, from_=0, to=23, width=3, format="%02.0f", **spin_style)
    start_h.grid(row=0, column=1, padx=5)
    # ... repeat for other spinboxes ...

    # --- Submit Button ---
    btn_send = tk.Button(
        root, 
        text="LOG TO AZURE CORE", 
        command=lambda: send_to_api(cal, start_h, start_m, end_h, end_m, entry_cons_id, entry_cust_id, var_lunch),
        bg=accent_color, 
        fg=bg_color, 
        font=('Courier', 11, 'bold'),
        activebackground="#E3E5FA",
        relief="flat",
        padx=20,
        pady=10
    )
